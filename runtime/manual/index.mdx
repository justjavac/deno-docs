...
Deno 提供了一个兼容层，使您的代码能够使用 [Node.js 内置模块和来自 npm 的第三方模块](./node/index.md)。在您的代码中使用 Node 和 npm 模块看起来很像使用标准的 Deno 模块，除了在导入 Node 内置模块或 npm 模块时，您将使用 `node:` 或 `npm:` 指示符。
